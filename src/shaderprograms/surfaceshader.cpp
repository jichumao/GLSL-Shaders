...
SurfaceShader::SurfaceShader(OpenGLContext *context)
    : ShaderProgram(context),
      ...
      unifModel(-1), unifModelInvTr(-1), unifView(-1), unifProj(-1), u_CameraPos(-1)
{}
...
void SurfaceShader::setupMemberVars()
{
    ...
    u_CameraPos    = context->glGetUniformLocation(prog, "u_CameraPos");
...
void SurfaceShader::setCameraPosition(const glm::vec3& cameraPos) {
    ...
    if (u_CameraPos != -1) {
    context->glUniform3f(u_CameraPos, cameraPos.x, cameraPos.y, cameraPos.z);
    }
