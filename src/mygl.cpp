...
void MyGL::paintGL()
{
    glm::vec3 cameraPos = m_camera.eye;


    // Update the camera position in the shader
    mp_progSurfaceCurrent->setCameraPosition(cameraPos);
...
void MyGL::createShaders()
{
    ...
    std::shared_ptr<PostProcessShader> myshader = std::make_shared<PostProcessShader>(this);
    myshader->create(":/glsl/post/passthrough.vert.glsl",":/glsl/post/myshader.frag.glsl");
    m_postprocessShaders.push_back(myshader);
